...
public class CambriaEventPublisherTest {

    private CambriaEventPublisher eventPublisher;
    private CambriaBatchingPublisher cambriaPublisher;
    private CambriaPublishersCache cambriaPublishersCache;

    @Before
    public void setUp() {
        cambriaPublisher = mock(CambriaBatchingPublisher.class);
        cambriaPublishersCache = mock(CambriaPublishersCache.class);
        when(cambriaPublishersCache.getPublisher(anyString())).thenReturn(Option(cambriaPublisher));
        eventPublisher = new CambriaEventPublisher(cambriaPublishersCache, mock(Logger.class));
    }
    ...
    public void shouldSendEventToTopic() throws Exception {
        ...
        // when
        eventPublisher.sendEvent(event, streamId);
    ...
    public void shouldCloseConnectionWhenExceptionOccurred() throws Exception {
        ...
        // then
        verify(cambriaPublishersCache).closePublisherFor(streamId);
    }
}