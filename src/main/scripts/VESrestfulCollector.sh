...
startApp() {
    ...
    ${JAVA_EXECUTABLE} -cp "${APP_BASE_PATH}/etc:${APP_BASE_PATH}/lib/*" \
      -Xms256m -Xmx512m \
      -XX:ErrorFile=${LOGS_PATH}/java_error%p.log \
      -XX:+HeapDumpOnOutOfMemoryError \
      -Dhttps.protocols=TLSv1.1,TLSv1.2 \
      org.onap.dcae.commonFunction.CommonStartup $* & &>> ${MAIN_LOG_FILE}
}
...
collector_configupdate() {
    if [ -z "$CONSUL_HOST" ] || [ -z "$CONFIG_BINDING_SERVICE" ] || [ -z "$HOSTNAME" ]; then
        log "Using standard controller configuration"
    else
        cd ${APP_BASE_PATH}
        ${JAVA_EXECUTABLE} -cp "${CONFIG_FILES_PATH}:${LIBRARIES_PATH}/*" org.onap.dcae.controller.FetchDynamicConfig $*
        ...
        if [ -f /opt/app/KV-Configuration.json ]; then
            ...
            ${JAVA_EXECUTABLE} -cp "etc:lib/*"  $CONFIG_UPDATER $*
            if [ $? -ne 0 ]; then
...
case $1 in
  "start") collector_configupdate; startApp ;;
  "stop")  stopApp ;;
