...
public class CustomFilter extends GenericFilterBean {
  ...
  public void doFilter(ServletRequest servletRequest, ServletResponse servletResponse,
    ...
    String subjectDN = getSubjectDN(
        (X509Certificate[]) servletRequest.getAttribute(CERTIFICATE_X_509));

    Stream<String> lines = Files.lines(Paths.get(PROPERTY));

    if (lines.anyMatch(element -> Pattern.compile(element).matcher(subjectDN).find())) {
      filterChain.doFilter(servletRequest,servletResponse);
    } else {
      HttpServletResponse response = (HttpServletResponse) servletResponse;
      response.sendError(HttpServletResponse.SC_FORBIDDEN, String.format(MESSAGE, PROPERTY,
          Files.lines(Paths.get(PROPERTY)).collect(Collectors.joining(" "))));
    }
  }
