...
public class ServletConfig implements WebServerFactoryCustomizer<ConfigurableServletWebServerFactory> {
    ...
    private Map<String, AuthMethod> provideAuthConfigurations(ConfigurableServletWebServerFactory container) {
        ...
        authMethods.put(AuthMethodType.CERT_ONLY, new CertOnly(container, properties));
        authMethods.put(AuthMethodType.BASIC_AUTH, new BasicAuth(container, properties));
        authMethods.put(AuthMethodType.CERT_BASIC_AUTH, new CertBasicAuth(container, properties));
        authMethods.put(AuthMethodType.NO_AUTH, new NoAuth(container, properties));
        return authMethods;
    }
}