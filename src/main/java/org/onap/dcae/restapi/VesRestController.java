...
public class VesRestController {
    ...
    ResponseEntity<String> event(@RequestBody String event, @PathVariable String version, HttpServletRequest request) {
        if (settings.isSupportedVersion(version)) {
    ...
    private ResponseEntity<String> process(String events, String version, HttpServletRequest request, String type) {
        ...
        JSONArray arrayOfEvents = new EventUpdater(settings).convert(jsonObject,version, generateUUID(version, request, jsonObject), type);
        eventSender.send(arrayOfEvents, metricsLog);
    ...
    private UUID generateUUID(String version, HttpServletRequest request, JSONObject jsonObject) {
