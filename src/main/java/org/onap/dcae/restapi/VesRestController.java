...
public class VesRestController {
    ...
    private ResponseEntity<String> process(String events, String version, HttpServletRequest request, String type) {
        ...
        if(headerUtils.isOkCustomHeaders()){
            try {
                ...
                validateEvent(version, type, jsonObject);
                List<VesEvent> vesEvents = transformEvent(version, request, type, jsonObject);
    ...
    private void validateEvent(String version, String type, JSONObject jsonObject) throws EventValidatorException {
        EventValidator eventValidator = new EventValidator(settings);
        eventValidator.validate(jsonObject, type, version);
    }

    private List<VesEvent> transformEvent(String version, HttpServletRequest request, String type, JSONObject jsonObject) {
        return new EventUpdater(settings).convert(
                jsonObject,version, generateUUID(version, request.getRequestURI(), jsonObject), type);
    }
