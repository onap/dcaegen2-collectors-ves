...
public class EventSender {
  ...
  private JSONObject overrideEvent(JSONObject event) {
    JSONObject object = addCurrentTimeToEvent(event);
    if (properties.eventTransformingEnabled()) {
      try (FileReader fr = new FileReader("./etc/eventTransform.json")) {
        ...
        parseEventsJson(events, new ConfigProcessorAdapter(new ConfigProcessors(object)));
      } catch (IOException e) {
        log.error("Couldn't find file ./etc/eventTransform.json" + e.toString());
      }
    }
    if (object.has("VESversion"))
      object.remove("VESversion");

    log.debug("Modified event:" + object);
