...
final class DMaaPConfigurationParser {
    ...
    static Map<String, PublisherConfig> parseDMaaPConfig(Path dMaaPConfigLocation) {
        LOG.info(String.format("Building DMaaP config using configuration file: '%s'", dMaaPConfigLocation));
        return Try(() -> AnyNode.fromString(new String(Files.readAllBytes(dMaaPConfigLocation))))
            ...
            .mapFailure(Case($(), e -> new RuntimeException(f("Could not load DMaaPConfiguration from file: '%s'", dMaaPConfigLocation), e)))
            .flatMap(jsonNode -> Try(() -> usesLegacyFormat(jsonNode) ? parseLegacyFormat(jsonNode) : parseNewFormat(jsonNode)))
            .mapFailure(Case($(), e -> new RuntimeException("Parsing DMaaP configuration failed, probably unexpected format occurred", e)))
