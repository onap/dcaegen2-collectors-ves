...
class DMaaPEventPublisher implements EventPublisher {
    ...
    public void sendEvent(JSONObject event, String domain) {
        ...
        publishersCache.getPublisher(domain)
            .onEmpty(() -> log.warn(f("Could not find event publisher for domain: '%s', dropping message: '%s'", domain, event)))
    ...
    private void uncheckedSendEvent(JSONObject event, String domain, CambriaBatchingPublisher publisher) throws IOException {
    ...
    private void closePublisher(JSONObject event, String domain, Throwable e) {
        log.error(f("Unable to publish event: '%s' on domain: '%s'. Closing publisher", event, domain), e);
