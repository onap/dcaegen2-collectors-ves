...
class DMaaPPublishersCache {
    ...
    void reconfigure(Map<String, PublisherConfig> newDMaaPConfiguration) {
        dMaaPConfiguration.getAndUpdate(oldConfig -> {
            Map<String, PublisherConfig> removed = oldConfig.filterKeys(domain -> !newDMaaPConfiguration.containsKey(domain));
            Map<String, PublisherConfig> changed = newDMaaPConfiguration
                .filterKeys(e -> oldConfig.containsKey(e) && !newDMaaPConfiguration.get(e).equals(oldConfig.get(e)));
            removed.merge(changed).forEach(e -> publishersCache.invalidate(e._1));
            return newDMaaPConfiguration;
        });
    }
    ...
    class CambriaPublishersCacheLoader extends CacheLoader<String, CambriaBatchingPublisher> {
        ...
        public CambriaBatchingPublisher load(@Nonnull String domain) {
            return dMaaPConfiguration.get()
                .get(domain)
                .toTry(() -> new RuntimeException(f("DMaaP configuration contains no configuration for domain: '%s'", domain)))
