...
class DMaaPPublishersCache {
    ...
    private Map<String, PublisherConfig> dMaaPConfiguration;

    DMaaPPublishersCache(Map<String, PublisherConfig> dMaaPConfiguration) {
        this.dMaaPConfiguration = dMaaPConfiguration;
    ...
    /*
     A client should not need to wait until the publisher is closed, as he does not wait for result of this operation.
    */
    void closePublisherFor(String streamId) {
        new Thread(() -> publishersCache.invalidate(streamId)).start();
    }

    /*
     This thing is designed to be called once in a while by a single thread, there will be no concurrent calls
     to this method. Underlying Guava's cache is already thread-safe
     so there is no need to introduce any synchronization here as the lists are immutable anyway.
    */
    void reconfigure(Map<String, PublisherConfig> newDMaaPConfiguration) {
        Map<String, PublisherConfig> removed = dMaaPConfiguration
            .filterKeys(domain -> !newDMaaPConfiguration.containsKey(domain));
        Map<String, PublisherConfig> changed = newDMaaPConfiguration
            .filterKeys(e -> {
                Option<PublisherConfig> newConfig = newDMaaPConfiguration.get(e);
                Option<PublisherConfig> oldConfig = dMaaPConfiguration.get(e);
                return dMaaPConfiguration.containsKey(e) && !newConfig.equals(oldConfig);
            });
        removed.merge(changed).forEach(e -> publishersCache.invalidate(e._1));
        dMaaPConfiguration = newDMaaPConfiguration;
    }
